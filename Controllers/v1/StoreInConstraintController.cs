...
namespace WarehouseWebApi.Controllers
{
    ...
    public class StoreInConstraintController : ControllerBase
    {
        ...
        public IActionResult Get(int companyID, int depoID)
        {
            ...
            var receives = new List<StoreInConstrainModel.M_StoreInConstraint>();
            try
            {
                receives = StoreInConstrainModel.GetStoreInConstraint(databaseName, depoID);
            }
            catch (Exception ex)
            {
            ...
            return receives.Count() == 0 ? Responce.ExNotFound("在庫入庫制御情報の取得に失敗しました") : Ok(receives);
        }
